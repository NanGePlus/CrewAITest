...
async def chat_completions(request: ChatCompletionRequest):
    ...
    try:
        ...
        # llm，指定crew中的Agent使用什么大模型
        # result = CrewtestprojectCrew(llm).leadMarketAnalystCrew().kickoff(inputs=inputs)

        # Execute the pipeline, initiating a run for each input
        results = await CrewtestprojectCrew(llm).my_pipeline().process_runs(inputs)
        # Access the results
